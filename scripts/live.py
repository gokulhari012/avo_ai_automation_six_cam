...
# Open webcam feed (default webcam index 0)
...
def apply_threshold(hsv, frame_copy, hsv_image):
...
while True:
    ...
    # Crop the frame if needed (as in your original code)
    framea = frame

    # Convert the image to HSV
    hsv_image = cv2.cvtColor(framea, cv2.COLOR_BGR2HSV)

    # Create a copy of the original frame for each window
    ...
    for i in range(4):
        ...
        threshold_image = apply_threshold(hsv_values[i], frames[i], hsv_image)
