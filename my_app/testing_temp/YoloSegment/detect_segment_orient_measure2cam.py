    ...
    if boxes is not None:
        ...
        if masks is not None:
            for mask in masks:
                ...
                cv2.imshow("Mask_"+str(cam_id),cv2.resize(processed_mask, (int(width/downscale),int(height/downscale))))

    cv2.imshow("Live Camera Feed"+str(cam_id),cv2.resize(frame, (int(width/downscale),int(height/downscale))))
